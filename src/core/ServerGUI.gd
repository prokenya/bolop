...
@export var player_name_line: LineEdit
...
func host_game():
	mpc._join_handshake_data["player_name"] = player_name_line.text
	mpc.start_online_host(true,{"player_name" : player_name_line.text})
	open_connecting_ui()
...
func join_game():
	...
	mpc.start_online_join(url,{"player_name" : player_name_line.text})
	open_connecting_ui()
